...
Conduit is a local OpenAI-compatible proxy that lets API-first AI coding tools talk to subscription-backed model auth. It was built first for Cursor's Agent harness and ChatGPT/Codex subscription auth, with streaming responses, tool calls, image inputs, prompt-cache visibility, and Cursor-readable usage reporting.
...
```powershell
conduit auth login --method device
```
...
Use the generated tunnel URL with `/codex` appended:
...
```text
USAGE: input=78008 cached=77952 output=214 reasoning=0 total=78222
```
...
```powershell
.\.venv\Scripts\python.exe -m pytest tests\test_conduit_auth.py tests\test_conduit_cli.py tests\test_codex_settings.py tests\test_codex_request_adapter.py tests\test_codex_response_adapter.py -q
```
...
Conduit's direct OpenAI Codex OAuth implementation is ported from [`badlogic/pi-mono`](https://github.com/badlogic/pi-mono), specifically its ChatGPT/Codex browser and device-code OAuth flow. We rewrote that behavior in Python for Conduit's CLI and `~/.conduit` auth state.
